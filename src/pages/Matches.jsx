    ...
    <div className="min-h-screen pt-20">
      ...
      <section className="py-10 sm:py-16 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center px-5 sm:px-4 relative z-10">
          <span className="inline-block text-primary font-display text-xs sm:text-sm tracking-widest mb-3 sm:mb-4">MATCHES</span>
          <h1 className="font-display font-900 text-3xl sm:text-6xl mb-3 sm:mb-4">
            赛事与<span className="text-primary">战绩</span>
          </h1>
          <p className="text-text-muted text-sm sm:text-base">每一场比赛，都是一次战斗</p>
        </div>
      </section>

      {/* Stats Overview */}
      <section className="py-6 sm:py-8 bg-bg-surface border-y border-border-subtle">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 sm:gap-4">
            {[
              ...
              <div key={i} className="text-center py-2 sm:py-3">
                ...
                <div className="text-text-dim text-[10px] sm:text-xs mt-0.5 sm:mt-1">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-3">
            {upcomingMatches.map((match) => (
              <div key={match.id} className="bg-bg-card border border-border-subtle rounded-lg p-3 sm:p-5 card-hover">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3">
                  <div>
                    <div className="flex items-center gap-2 text-text-dim text-xs mb-1.5 sm:mb-2">
                      <Calendar size={12} />
                    ...
                    <div className="text-text-main font-medium text-base sm:text-lg">
            ...
            {matches.map((match) => (
              <div key={match.id} className="bg-bg-card border border-border-subtle rounded-lg p-3 sm:p-4 card-hover">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-text-dim text-xs flex items-center gap-1 sm:gap-2 truncate">
                  ...
                  <span className={`text-xs font-bold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded shrink-0 ${
                ...
                <div className="flex items-center justify-center gap-3 sm:gap-4">
                  <span className={`text-sm sm:text-base font-medium truncate ${match.home ? 'text-primary' : 'text-text-muted'}`}>
                    {match.home ? 'TinmanFC' : match.opponent}
                  </span>
                  <span className="font-display font-bold text-lg sm:text-xl text-text-main px-2 shrink-0">{match.score}</span>
                  <span className={`text-sm sm:text-base font-medium truncate text-right ${!match.home ? 'text-primary' : 'text-text-muted'}`}>
                    {!match.home ? 'TinmanFC' : match.opponent}
                  </span>
                </div>
